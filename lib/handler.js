  ...
  handler.item = function (req, res, options) {
    ...
    if (req.method === 'PUT') {
      jsonBody(req, res, function (err, body) {
        media.get(options.params.key, function (err, mediaModel) {
          store.exists({key: mediaModel.name}, function (err, exists) {
            if (err) return response.error(err).pipe(res)
            ...
            if (store.s3 && store.bucket) {
              ...
              s3.copyObject(params, function (err, data) {
                ...
                media.update({key: options.params.key, name: body.name, path: newName}, function (err, mediaModel) {
                  if (err) return response.error(err).pipe(res)
                  return response.json(mediaModel).pipe(res)
                })
              })
            ...
            // Disk Storage
            var oldName = staticFileDir + '/' + uploadDir + '/' + mediaModel.name
            var newName = staticFileDir + '/' + uploadDir + '/' + body.name
    ...
    if (req.method === 'DELETE') {
      ...
      media.get(modelKey, function (err, mediaModel) {
        ...
        // Disk Storage
        var filePath = staticFileDir + '/' + uploadDir + '/' + mediaModel.name
