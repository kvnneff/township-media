...
function handler (media, options, app) {
  ...
  var maxFileSize = options.maxFileSize || 2000000
  ...
  function checkMimeType (mimetype) {
    var allowed = false
    if (allowedMimeTypes.length === 0 && mime.extension(mimetype)) { 
      return true
    }
    allowedMimeTypes.some(function (type) {
      if (mime.lookup(type) !== mimetype) return false
      allowed = true
      return allowed
    })
    return allowed
  ...
  function handlePost (request, response) {
    ...
    /**
     * Return the array of media models
     * @api private
     */
    function done() {
    ...
    /**
     * Create a new write stream and create a new media model
     * on successful writes.
     * @param  {String} key      Blob store key
    ...
    busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
      if (!checkMimeType(mimetype)) {
        return res.error('Not a valid mimetype').pipe(response)
      }
    ...
    busboy.on('limit', function () {
      return res.error('File size exceeds max file size').pipe(response)
    })
