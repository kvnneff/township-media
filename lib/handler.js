...
function handler (media, options) {
  var options = options || {}
  var uploadDir = options.uploadDir || process.cwd() + '/media_uploads'
  var store = null
  var handler = {}
  ...
  handler.item = function (req, res, options) {
    ...
    if (req.method === 'PUT') {
      ...
      busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        var writeStream = store.createWriteStream({key: filename}, function (err, opts) {
          ...
          media.update(options.params.key, newAttributes, function (err, media) {
            if (err) return response.error(err).pipe(res)
    ...
    if (req.method === 'DELETE') {
      ...
      media.get(modelKey, function (err, mediaModel) {
        ...
        // Disk Storage
        var filePath = uploadDir + '/' + mediaModel.name
